...
class Librarian
{
	public:
		...
		Book createBookRecord(std::string p_AuthorsSurname, std::string p_AuthorsName, std::string p_Title, int p_BooksAmount);
		void getBookData(std::string& p_AuthorsSurname, std::string& p_AuthorsName, std::string& p_Title, int& p_Amount);
		void addBook(std::string p_AuthorsSurname, std::string p_AuthorsName, std::string p_Title, std::vector<Book>& p_Books);
	private:
		std::string mName;
		std::string mSurname;
		static int mID;
